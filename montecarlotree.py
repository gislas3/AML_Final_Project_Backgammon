...
class MonteCarloTree:
    ...
    def __init__(self, start_state, C, ta):
        ...
        bstring = "+" + bstring
        try:
            with open("backgammon_mct2.p", "rb") as f:
        ...
        # normal, somewhat expected
            ...
            self.statedict = {bstring: {"total_visits": 0, "turn" : 1}}#corresponds to reward, visit, turn
            #continue
        self.visited = []
        self.added = 0
        self.its = 0
        ...
        self.currturn = self.statedict[bstring]["turn"] #n
        self.move = ()
    ...
    def reset(self, brd):
        ...
        self.its = 0
        self.prevstate = bstring
        self.currturn =self.statedict[bstring]["turn"]
        self.move = ()
        self.currstate = bstring
        self.dlist = ()

    def makeMove(self, listmoves, dicelist):
        self.dlist = dicelist
        if(self.statedict.has_key(self.currstate)): #state is in table of states
            ...
            if(self.statedict[self.currstate].has_key(dicelist)): #have seen dice roll before
                if(len(listmoves) == len(self.statedict[self.currstate][dicelist])): #have tried every possible move from state and dice combination
                    maxval = -100000
                    maxmove = ()
    ...
    def savedict(self):
        pickle.dump( self.statedict, open( "backgammon_mct2.p", "wb" ) )
